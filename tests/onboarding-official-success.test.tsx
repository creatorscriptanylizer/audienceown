...
describe("official account onboarding success",()=>{
  it("keeps the setup state until a real official account is connected",()=>{const html=render([]);expect(html).toContain("Where followers know you");expect(html).toContain("Connect your official account");expect(html).not.toContain("Your main account is connected");});
  it("renders the complete success state from connected account data",()=>{const html=render([account("youtube","KwaMoon")]);expect(html).toContain("Main account connected");expect(html).toContain("Your main account is connected");expect(html).toContain("Your audience now has a trusted account to recognize you from.");expect(html).toContain("YouTube");expect(html).toContain("KwaMoon");expect(html).toContain("Official account");expect(html).toContain("Connected");expect(html).toContain("Change account");expect(html).toContain("Continue to backup account");expect(html).toContain('name="step" value="official"');expect(html).not.toContain("Connect your official account");});
  ...
  it("ignores disconnected, archived, and revoked records",()=>{for(const state of [{connected:false},{archived:true},{revoked:true}]){const html=render([{...account("youtube","KwaMoon"),...state}]);expect(html).toContain("Connect your official account");expect(html).not.toContain("Your main account is connected");}});
  it("uses server-provided OAuth-return data immediately",()=>{const html=render([account("youtube","OAuth Channel")]);expect(html).toContain("OAuth Channel");expect(html).toContain("Continue to backup account");});
