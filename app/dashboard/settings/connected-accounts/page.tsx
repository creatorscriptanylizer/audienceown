...
export default async function ConnectedAccountsPage(){
  ...
  return <><header><p className="eyebrow">Settings</p><h1 className="mt-2 text-3xl font-semibold">Connected Platforms</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-zinc-400">See every connected account, its current health, and whether you need to take action.</p></header>
    {!providerResult?<section role="alert" className="mt-8 rounded-2xl border border-amber-300/20 bg-amber-300/5 p-6"><CircleAlert aria-hidden className="text-amber-300"/><h2 className="mt-4 font-semibold text-amber-100">Connections could not be loaded</h2><p className="mt-2 text-sm leading-6 text-zinc-400">Your saved accounts were not changed. Refresh the page or try again later.</p></section>:cards.length===0?<section className="surface mt-8 rounded-2xl border-dashed p-7 text-center"><span className="mx-auto grid size-12 place-items-center rounded-2xl bg-violet-400/10 text-violet-300"><Link2 aria-hidden size={21}/></span><h2 className="mt-5 text-lg font-semibold">Connect your first platform</h2><p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-zinc-400">Connect your first platform to begin synchronizing your creator presence, or save a public profile as a manual account.</p><div className="mt-5 flex flex-col justify-center gap-3 sm:flex-row"><Link prefetch={false} className="button button-primary min-h-11" href="/api/integrations/youtube/connect?role=official">Connect YouTube</Link><Link className="button button-secondary min-h-11" href="/dashboard/platforms">Add a public URL</Link></div></section>:<section className="mt-8" aria-labelledby="connection-overview-heading"><div className="flex flex-wrap items-end justify-between gap-4"><div><h2 id="connection-overview-heading" className="text-xl font-semibold">Your connections</h2><p className="mt-2 text-sm text-zinc-500">{cards.length} saved {cards.length===1?"account":"accounts"}</p></div><Link href="/dashboard/platforms" className="button button-secondary min-h-11">Add or edit accounts <ArrowUpRight aria-hidden size={15}/></Link></div><div className="mt-5 grid gap-4 2xl:grid-cols-2">{cards.map((connection)=><ConnectedPlatformCard key={connection.id} connection={connection}/>)}</div></section>}
    {youtube&&<div className="mt-10"><YouTubeConnectionManager connection={youtube}/></div>}
