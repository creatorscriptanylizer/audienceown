...
export async function activateAndDeliverEmergency(emergencyId:string,client:SupabaseClient<Database>){
const{data,error}=await client.rpc("activate_emergency",{p_emergency_id:emergencyId});if(error)throw error;
