...
const ROUTE = "provider_analytics_expansion_three";
const providers = ["x", "linkedin"] as const;
...
export async function GET() {
  ...
  const [detectionsResult, updatesResult, connectionsResult, assetsResult] = await Promise.all([
    ...
    db.from("provider_asset_bindings").select("provider,last_successful_sync_at").eq("creator_id", creator.id).in("provider", providers),
  ]);
  ...
  return Response.json({ providers: Object.fromEntries(providers.map((provider) => {
    const found = detections.filter((item) => item.provider === provider);
    const drafts = updates.filter((item) => item.source_provider === provider);
    return [provider, {
      detected: found.length, automatic: found.filter((item) => item.detection_source !== "manual_provider_import").length,
      manual: found.filter((item) => item.detection_source === "manual_provider_import").length,
      draftsGenerated: drafts.length, sent: drafts.filter((item) => item.status === "sent").length,
      ...
      lastSync: connections.filter((item) => item.platform === provider).map((item) => item.last_sync_at).filter(Boolean).sort().at(-1) ?? null,
    }];
