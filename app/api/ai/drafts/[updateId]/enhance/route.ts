...
if(!creator)return Response.json({error:"Unauthorized"},{status:401});const client=await createClient();const{data:owned,error:ownedError}=await client!.from("creator_updates").select("id").eq("id",updateId).eq("creator_id",creator.id).eq("status","draft").maybeSingle();
if(ownedError)return apiUnavailable("POST /api/ai/drafts/[updateId]/enhance","creator_updates",ownedError);
