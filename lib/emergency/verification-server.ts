...
export async function verifyFromConnectedAccount(input:{creatorId:string;userId:string;emergencyId:string;replacementId:string;connectionId:string}){
 ...
 if(error)throw error;return{id:data,identity:{provider:connection.platform,externalAccountId:identity.id,name:identity.name,url:identity.url},confidence:"high"};
}
