...
type EmailVerificationUnavailableReason = "database_admin_not_configured" | "email_otp_pepper_not_configured" | "contact_encryption_not_configured" | "email_provider_not_configured" | "email_sender_not_configured" | "provider_rejected";

function unavailableReason(adminReady: boolean, pepper: string | undefined, contactEncryptionKey: string | undefined, resendKey: string | undefined, from: string | undefined): EmailVerificationUnavailableReason | null {
  if (!adminReady) return "database_admin_not_configured";
  if (!pepper || pepper.length < 20) return "email_otp_pepper_not_configured";
  if (!contactEncryptionKey || contactEncryptionKey.length < 20) return "contact_encryption_not_configured";
...
export async function POST(request: Request) {
  try {
    ...
    const configurationFailure = unavailableReason(Boolean(admin), pepper, contactEncryptionKey, resendKey, from);
    if (configurationFailure) {
      ...
      return response({ kind: "verification_unavailable", reason: configurationFailure, title: "We Couldn't Verify Your Email Right Now", message: "Your information is safe. Please try again." }, 503);
    }
    if (input.action === "verify") {
      ...
      if (challenge?.completed_at) return response({ kind: "validation_error", code: "consumed", message: "This verification link has already been used." }, 409);
      if (!challenge || challenge.cancelled_at || challenge.replaced_at || challenge.verified_at || new Date(String(challenge.expires_at)) <= new Date()) return response({ kind: "validation_error", code: "expired", title: "Your Verification Link Expired", message: "Send a new verification email to continue." }, 400);
      ...
      if (completed.error) return response({ kind: "system_error", title: "We Couldn't Verify Your Email Right Now", message: "Your information is safe. Please try again." }, 503);
    ...
    if (inserted.error) return response({ kind: "system_error", title: "We Couldn't Verify Your Email Right Now", message: "Your information is safe. Please try again." }, 503);
    ...
    if (sent.error) {
      ...
      return response({ kind: "verification_unavailable", reason: "provider_rejected", title: "We Couldn't Verify Your Email Right Now", message: "Your information is safe. Please try again." }, 503);
    }
    ...
    if (error instanceof z.ZodError) return response({ kind: "validation_error", message: "Check the verification details and try again." }, 400);
    return response({ kind: "system_error", title: "We Couldn't Verify Your Email Right Now", message: "Your information is safe. Please try again." }, 503);
  }
}
