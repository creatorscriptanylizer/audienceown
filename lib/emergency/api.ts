...
if(!user||!client)return null;if(creator)return{creator,user,client};const{data:membership,error:membershipError}=await client.from("creator_team_members").select("creator_id").eq("user_id",user.id).limit(1).maybeSingle();if(membershipError)throw new Error("emergency_membership_unavailable",{cause:membershipError});
...
if(error?.code==="23505")return Response.json({error:"Emergency resource already exists"},{status:409});
