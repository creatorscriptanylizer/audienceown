...
export function SubscribeForm({handle,creatorName,source}:{handle:string;creatorName:string;source?:string}) {
  ...
  async function submit(e:React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response=await fetch(`${base}/functions/v1/subscribe`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({
        slug:handle,email:form.get("email"),consent:true,source_platform:normaliseSource(source),
        landing_path:window.location.pathname,source_referrer:document.referrer||null,
        preferences:{recovery:true,videos:form.get("videos")==="on",livestreams:form.get("livestreams")==="on",announcements:form.get("announcements")==="on",products:form.get("products")==="on"}
      })});
  ...
  return <form onSubmit={submit} className="pass-flow">
    ...
    <div><p className="eyebrow">Your pass</p><h3>Where should we reach you?</h3><label className="label" htmlFor="pass-email">Email address</label><div className="relative"><Mail className="absolute left-3 top-3.5 text-zinc-500" size={17}/><input className="input pl-10" id="pass-email" name="email" type="email" inputMode="email" autoComplete="email" required/></div><fieldset className="mt-6"><legend className="label">Choose your alerts</legend><div className="pass-options"><div><ShieldCheck size={16}/><span><b>Recovery alerts · Required</b><small>Account unavailable, hacked, suspended, deleted, or moved.</small></span></div><label><input type="checkbox" name="videos"/><span><b>New videos</b></span></label><label><input type="checkbox" name="livestreams"/><span><b>Live streams</b></span></label><label><input type="checkbox" name="announcements"/><span><b>Announcements</b></span></label><label><input type="checkbox" name="products"/><span><b>Official products</b></span></label></div></fieldset>{state==="error"&&<p role="alert" className="mt-3 text-sm text-red-300">We couldn’t activate your pass. Please try again.</p>}<button disabled={state==="loading"} className="button button-primary mt-5 w-full">{state==="loading"&&<LoaderCircle className="animate-spin" size={16}/>}Activate Recovery Pass</button><p className="mt-3 text-xs text-zinc-500">Recovery alerts are included. Manage or deactivate your pass at any time.</p></div>}
  </form>;
}
