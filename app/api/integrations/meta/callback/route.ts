...
export async function GET(request:Request){
  ...
  const finish=(status:string,connectionId?:string)=>{const destination=new URL(state.returnTo==="onboarding"?`/onboarding/accounts?step=${state.role}&oauth=facebook:${status}`:`/dashboard/platforms?social=facebook:${status}`,canonicalOAuthOrigin()??request.url);if(connectionId&&state.returnTo!=="onboarding")destination.searchParams.set("connectionId",connectionId);return NextResponse.redirect(destination);};
  ...
  try{
    ...
    const appUser=await fetchMetaAppUser(tokens.accessToken),values={creator_id:creator.id,platform:"facebook" as const,account_type:state.role,protected_official_account_id:state.role==="backup"?state.protectedOfficialAccountId??null:null,label:"Meta asset access",url:"https://www.facebook.com/",is_primary:false,is_public:false,external_account_id:null,external_account_name:appUser.name,external_account_url:null,provider_metadata:{loginMethod:"facebook_login",assetSelectionRequired:true,reconnectTargetId:state.connectionId??null,protectedOfficialAccountId:state.protectedOfficialAccountId??null,recoveryForMainAccountId:state.recoveryForMainAccountId??null},requested_scopes:facebookProvider.requestedScopes,granted_scopes:tokens.grantedScopes,watch_enabled:false,auto_create_drafts:true,auto_send:false,webhook_enabled:false,connection_health:"healthy",provider_status:"asset_selection_required",token_expires_at:tokens.expiresAt,token_refreshed_at:new Date().toISOString()};
    ...
    if(assets.length===1){await selectMetaAsset(creator.id,insertedId,"facebook",assets[0].selectionKey);const autoLink=await applyRecoveryAutoLinkIntent(db,{creatorId:creator.id,connectedAccountId:insertedId,provider:"facebook",role:state.role,recoveryForMainAccountId:state.recoveryForMainAccountId});callback.success({connectionId:insertedId,role:state.role,automaticSelection:true});return finish(autoLink==="linked"||autoLink==="existing"?"connected_recovery_linked":autoLink==="failed"?"connected_recovery_link_failed":"connected",insertedId);}
