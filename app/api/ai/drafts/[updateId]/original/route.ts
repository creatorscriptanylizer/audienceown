...
const client=await createClient();const{data:update,error:updateError}=await client!.from("creator_updates").select("deterministic_title,deterministic_content").eq("id",updateId).eq("creator_id",creator.id).eq("status","draft").maybeSingle();
if(updateError)return apiUnavailable("POST /api/ai/drafts/[updateId]/original","creator_updates",updateError);
