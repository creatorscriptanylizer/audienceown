...
import { ProviderReliabilityError } from "../errors";
import { identity } from "../base-oauth";

export const youtubeProvider: SocialProviderAdapter = {
  provider:"youtube", pkce:false, displayName:"YouTube", availability:"implemented_credentials_required",
  ...
  async exchangeAuthorizationCode({code}) {
    const token=await exchangeYouTubeCode(code),scopes=(token.scope??"").split(" ").filter(Boolean); return {accessToken:token.access_token,refreshToken:token.refresh_token,
      expiresAt:new Date(Date.now()+token.expires_in*1000).toISOString(),scopes,grantedScopes:scopes,tokenType:token.token_type??"Bearer"};
  },
  async refreshAccessToken({refreshToken}) {
    if(!refreshToken) throw new ProviderReliabilityError("authentication","missing_refresh_token",false,true); const token=await refreshYouTubeAccessToken(refreshToken);
    const scopes=(token.scope??"").split(" ").filter(Boolean);return {accessToken:token.access_token,refreshToken:token.refresh_token,expiresAt:new Date(Date.now()+token.expires_in*1000).toISOString(),
      scopes,grantedScopes:scopes,tokenType:token.token_type??"Bearer"};
  },
  async fetchIdentity({accessToken}) { const channel=await getYouTubeChannel(accessToken);
    return identity(channel.id,channel.title,`https://www.youtube.com/channel/${channel.id}`,{uploads_playlist_id:channel.uploadsPlaylistId}); },
