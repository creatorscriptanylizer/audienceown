...
describe("dashboard Recovery Pass behavior",()=>{
  ...
  it("uses responsive-safe URL and action styling",()=>{const source=readFileSync("components/dashboard/recovery-pass-card.tsx","utf8");expect(source).toContain("overflow-hidden text-ellipsis whitespace-nowrap");expect(source).toContain("flex-wrap");expect(source).toContain("min-h-11")});
});
