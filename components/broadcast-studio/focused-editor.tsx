...
export function FocusedEditor({ step, title, description, children, onClose, closeDisabled = false, onContinue, continueLabel = "Save and continue", continueDisabled = false, hideContinue = false, footer }: {
  step: string; title: string; description: string; children: React.ReactNode;
  onClose: () => void; closeDisabled?: boolean; onContinue: () => void; continueLabel?: string; continueDisabled?: boolean; hideContinue?: boolean; footer?: React.ReactNode;
}) {
  ...
  return <div className="studio-focus-backdrop" role="presentation" onMouseDown={() => { if (!closeDisabled) onClose(); }}>
    <section ref={panelRef} className="studio-focus-panel" role="dialog" aria-modal="true" aria-labelledby="studio-focus-title" onMouseDown={(event) => event.stopPropagation()}>
      <header className="studio-focus-header">
        ...
        <button type="button" className="studio-focus-close" onClick={onClose} disabled={closeDisabled} aria-label={`Close ${title}`}><X/></button>
      </header>
