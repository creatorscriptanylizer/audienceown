...
export function AuthForm({ action, mode, next }: { action: (state: AuthState, data: FormData) => Promise<AuthState>; mode: "login" | "signup" | "forgot" | "reset"; next?: string }) {
  ...
  return <form action={formAction} className="space-y-4">
    {mode === "login" && <input type="hidden" name="next" value={next ?? "/dashboard"}/>}
    ...
    <SubmitButton pendingText={mode === "login" ? "Signing in…" : mode === "signup" ? "Creating your page…" : mode === "forgot" ? "Sending reset link…" : "Updating password…"}>{mode === "login" ? "Sign in" : mode === "signup" ? "Create your page" : mode === "forgot" ? "Send reset link" : "Update password"}</SubmitButton>
  </form>;
}
