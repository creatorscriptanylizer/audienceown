...
export function SocialProviderGrid({providers,connections,accountProjection=[]}:{providers:Definition[];connections:Connection[];accountProjection?:CreatorProviderAccount[]}){
 ...
 <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{providers.map((definition)=>{const connection=byProvider.get(definition.provider),projected=accountProjection.filter((account)=>account.provider===definition.provider&&!account.archived),connected=projected.some((account)=>account.connected);
  const detection=definition.capabilities.contentDetection;return <article className="surface rounded-2xl p-5" key={definition.provider}>
   <div className="flex items-start justify-between gap-3"><div><h3 className="text-lg font-semibold">{definition.displayName}</h3>
    ...
    {connected?<span className="flex items-center gap-1 text-xs text-emerald-300"><CheckCircle2 size={13}/>{projected.some((account)=>account.verified)?"verified":"connected"}</span>:
   ...
   <div className="mt-4 flex flex-wrap gap-2">{connected?<><Link className="button button-secondary" href={`/api/integrations/${definition.provider}/reconnect`}>Reconnect</Link>
