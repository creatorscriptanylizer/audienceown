...
const startSchema = z.object({
  ...
  consentVersion: z.literal("whatsapp-recovery-v1"),
  ...
  try {
    ...
    if (input.action === "start") {
      ...
      if (!method || method.method_status !== "verified") {
        const pendingMethod = await admin.from("follower_recovery_methods")
          .upsert({
            ...
            consent_purpose: "recovery_alerts_whatsapp",
      ...
      const [{ count: destinationCount }, { count: ipCount }] = await Promise.all([
        admin.from("whatsapp_verification_sessions").select("*", { count: "exact", head: true })
      ...
      await admin.from("whatsapp_verification_sessions")
      ...
      const { error } = await admin.from("whatsapp_verification_sessions").insert({
    ...
    if (input.action === "remove") {
      ...
      const { data: method } = await admin.from("follower_recovery_methods")
        ...
        .eq("method_type", "whatsapp").maybeSingle();
      ...
      await admin.from("whatsapp_verification_sessions").update({ replaced_at: now.toISOString() })
        .eq("recovery_method_id", method.id).is("completed_at", null).is("replaced_at", null);
      return json({ status: "removed" });
    }

    const { data: session } = await admin.from("whatsapp_verification_sessions")
      .select("id,recovery_method_id,creator_id,expires_at,resend_available_at,resend_count,attempt_count,completed_at,cancelled_at,replaced_at,preferences,source_platform,source_referrer,landing_path,follower_recovery_methods(follower_contact_id,destination_hash,destination_masked)")
      .eq("session_token_hash", await sha256(input.sessionToken)).maybeSingle();
    if (!session || session.completed_at || session.cancelled_at || session.replaced_at) return json({ status: "expired_code" }, 410);
    ...
    if (input.action === "cancel") {
      await admin.from("whatsapp_verification_sessions").update({ cancelled_at: now.toISOString() })
        .eq("id", session.id);
    ...
    if (input.action === "resend") {
      ...
      const sent = await twilioVerifyRequest("Verifications", new URLSearchParams({ To: phone, Channel: "whatsapp" }));
      if (!sent.ok) return json({ status: sent.code }, 503);
      await admin.from("whatsapp_verification_sessions").update({
    ...
    const { error: verifiedError } = await admin.from("whatsapp_verification_sessions")
      .update({ provider_verified_at: now.toISOString() })
      .eq("id", session.id)
      .is("completed_at", null)
    ...
    const { error: activationError } = await admin.rpc("complete_whatsapp_recovery_verification", {
      p_session_id: session.id,
      p_recovery_method_id: session.recovery_method_id,
      p_destination_hash: method.destination_hash,
