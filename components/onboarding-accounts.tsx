...
export function OnboardingAccounts({role,accounts,capabilities,entitlements,oauthMessage}:Props){
  ...
  return <section className={`onboarding-accounts${connectedSuccess?" is-success":""}`}><header>{connectedSuccess&&<span className="onboarding-success-icon"><ShieldCheck aria-hidden size={26}/><span className="sr-only">Success</span></span>}<p className={`eyebrow${connectedSuccess?" onboarding-success-eyebrow":""}`}>{officialConnected?"Main account connected":backupConnected?"Backup account connected":role==="official"?"Where followers know you":"Where followers can find you next"}</p><h1>{officialConnected?"Your main account is connected":backupConnected?"Your backup account is connected":role==="official"?"Connect your official account":"Add a backup account"}</h1><p>{officialConnected?"Your audience now has a trusted account to recognize you from.":backupConnected?"Your audience now has another trusted place to find you if your main account becomes unavailable.":role==="official"?"This is the account your followers already know and trust.":"Add another place followers can find you if your official account becomes unavailable."}</p></header>
    {oauthMessage&&<p className="onboarding-oauth-message" role="alert">{oauthMessage}</p>}
    {connected.length>0?<><div className="onboarding-connected-grid">{connected.map(account=><article key={account.accountKey} className="onboarding-connected-card"><PlatformBrandIcon provider={account.provider} label={account.provider}/><div><span>{capabilityMap.get(account.provider)?.displayName??account.provider}</span><h2>{account.displayName}</h2>{account.handle&&account.handle!==account.displayName&&<p>{account.handle}</p>}<small>{role==="official"?"Official account":"Backup account"}</small></div><strong><Check aria-hidden size={14}/> Connected</strong></article>)}</div><details className="onboarding-change-account"><summary>Change account</summary>{picker}</details></>:picker}
    <form action={completeAccountStep} className="onboarding-step-actions">{officialConnected&&<p className="onboarding-next-step">Next, add a backup account so your followers have another trusted place to find you.</p>}<input type="hidden" name="step" value={role}/><button className="button button-primary" type="submit">{officialConnected?"Continue to backup account":backupConnected?"Continue to finish":"Skip for now"}</button>{!connectedSuccess&&<p>You can connect this account later from Platforms.</p>}</form>
  </section>
}
