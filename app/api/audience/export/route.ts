...
export async function GET(){
  ...
  const{data:connections}=await admin.from("follower_connections").select("*").eq("creator_id",creator.id).order("consented_at");
  ...
  const[{data:contacts},{data:prefs}]=await Promise.all([
    contactIds.length?admin.from("follower_contacts").select("*").in("id",contactIds):Promise.resolve({data:[]}),
    connectionIds.length?admin.from("follower_notification_preferences").select("*").in("follower_connection_id",connectionIds):Promise.resolve({data:[]}),
  ]);
  ...
  return new NextResponse(`\uFEFF${lines.join("\r\n")}`,{headers:{"content-type":"text/csv; charset=utf-8","content-disposition":`attachment; filename="audienceown-${creator.public_slug}-protected-audience.csv"`,"cache-control":"no-store"}});
}
